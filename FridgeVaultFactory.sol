...
contract FridgeVault  {
    ...
    address public fridge_address; // FRIDGE token
    address public platformFeesCollector;
    address public fridge_bnb_lp_address;
    uint public softCap;
    uint public hardCap;
    uint public start_date;
    uint public end_date;
    uint public rate;               // coin sale rate 1 BNB = 1 FRIDGE (rate = 1e18) <=> 1 BNB = 10 FRIDGE (rate = 1e19) 
    uint public min_allowed;
    uint public max_allowed;        // Max BNB 
    uint public collected;          // collected BNB
    uint public pool_rate;          // uniswap liquidity pool rate  1 BNB = 1 FRIDGE (rate = 1e18) <=> 1 BNB = 10 FRIDGE (rate = 1e19)
    uint public lock_duration;      // duration wished to keep the LP tokens locked
    uint public withdrewToken;
    uint public pancake_rate;
    uint public prime_member_balance = 2000 * 1e18;
    ...
    function initilaize(uint[] calldata _data,address _token,address _owner_Address,uint _pool_rate,uint _lock_duration,uint _pancake_rate, address fridge_address_, address platformFeesCollector_, address fridge_bnb_lp_address_, address factory_owner_) external returns (uint){
        ...
        fridge_address = fridge_address_;
        platformFeesCollector = platformFeesCollector_;
        fridge_bnb_lp_address = fridge_bnb_lp_address_;
    ...
    function fridgeVAULT() public only_factory_authorised returns(uint){
    ...
    function addLiquidity() internal returns(bool){
        ...
        payable(platformFeesCollector).transfer(platform_fees.mul(400).div(1000));              // pool fees 40% of (2)% raised BNB 
            
        IPool(fridge_bnb_lp_address).notifyReward.value(platform_fees.mul(600).div(1000))();    // pool fees 60% of (2)% raised BNB
    ...
    // Fridge Holder ( > 2000) can participate 2 hour earlier in pre-sale
    function isLive() public view returns(bool){
        if((collected >= hardCap)) return false;
        if(collected >= softCap && block.timestamp >= end_date) return false;
        if(IBEP20(address(fridge_address)).balanceOf(msg.sender) >= uint(prime_member_balance)) {    // add condition if user may have 2000 FRIDGE   
    ...
    function exit() public {
        require(factory_owner == msg.sender, "error: authorised wallet");
 ...
 *
 * This implementation is agnostic to the way tokens are created. This means
 * t/hat a supply mechanism has to be added in a derived contract using {_mint}.
 * For a generic mechanism see {ERC20PresetMinterPauser}.
 *
 * TIP: For a detailed writeup see our guide
 * https://forum.zeppelin.solutions/t/how-to-implement-erc20-supply-mechanisms/226[How
...
// Name : FridgeVaultFactory
contract FridgeVaultFactory  {
    using Address for address;
    using SafeMath for uint;
    ...
    constructor(address _fridge_address,uint min_balance,uint _fee,address _pancakeRouter, address _platformFeesCollector, address _fridge_bnb_lp_address) public {
        factory_owner = msg.sender;
        toFee = msg.sender;
        fridge_address = _fridge_address;                  // FRIDGE Token Contract Address       
        balance_required = min_balance;                    // In percentage 50 / 10000
        fee = _fee;
        pancake_router = _pancakeRouter;                   // Pancake Router
        fridge_bnb_lp_address = _fridge_bnb_lp_address;
    ...
    //  1 BNB = 1 FRIDGE (_pool_rate = 1e18) <=> 1 BNB = 10 FRIDGE (_pool_rate = 1e19) <=> FRIDGE (decimals = 18)
    ...
    function createCampaign(uint[] memory _data,address _token,uint _pool_rate,uint _lock_duration,uint _pancake_ratio) external returns (address campaign_address){
        ...
        require(IBEP20(address(fridge_address)).balanceOf(msg.sender) >= _data[1].mul(_data[4]).mul(balance_required).div(1e23), "You don't have the minimum FRIDGE tokens required to launch a campaign");
        ...
        bytes memory bytecode = type(FridgeVault).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_token, msg.sender));
        assembly {
            campaign_address := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        
        FridgeVault(campaign_address).initilaize(_data,_token,msg.sender,_pool_rate,_lock_duration,_pancake_ratio, fridge_address, platformFeesCollector, fridge_bnb_lp_address, factory_owner);
